...
sudo wget https://dl.google.com/android/repository/android-ndk-r12b-linux-x86_64.zip
sudo unzip -q android-ndk-r12b-linux-x86_64.zip -d /usr/local/
sudo rm android-ndk-r12b-linux-x86_64.zip
...
cd /home
sudo wget https://gstreamer.freedesktop.org/data/pkg/android/1.10.5/gstreamer-1.0-android-universal-1.10.5.tar.bz2
sudo mkdir -p /usr/local/gstreamer
sudo tar -xjf gstreamer-1.0-android-universal-1.10.5.tar.bz2 -C /usr/local/gstreamer
sudo rm gstreamer-1.0-android-universal-1.10.5.tar.bz2
...
export PATH=${PATH}:/usr/local/android-ndk-r12b
